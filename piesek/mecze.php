...
function edytuj($mecz, $tabela = "mecze", $druzyna1, $druzyna2, $d1_kosze, $d2_kosze, $d1_wynik, $d2_wynik, $kwarty, $opis, $dzien, $miesiac, $rok, $rozgrywki, $sezon, $kolejka, $gracz_meczu, $gracz_meczu_czemu, $zobacz_tez, $nastepny, $poprzedni, $dodatkowe, $para, $godzina, $miejsce) {
   if(!$mecz) {
      $pobierz = mysql_query("select*from mecze where sezon='2005/2006' order by rozgrywki,kolejka");
...
function dopisz($czy, $akcja, $druzyna1, $druzyna2, $d1_kosze, $d2_kosze, $d1_wynik, $d2_wynik, $kwarty, $opis, $dzien, $miesiac, $rok, $rozgrywki, $sezon, $kolejka, $gracz_meczu, $gracz_meczu_czemu, $zobacz_tez, $dodatkowe, $para, $godzina, $miejsce, $ktore) {
   if(!$ktore) {
      ...
      echo "Do których rozgrywek...";
      $rozgryw = mysql_query("select*from rozgrywki order by sezon desc,rozgrywki");
      echo "<select name=ktore>";
      while ($rekordd = mysql_fetch_array($rozgryw)) {
         print "<option value=$rekordd[id]>$rekordd[rozgrywki] - $rekordd[sezon]</option>";
      }
      echo "</select>";
      print "<br><input type=submit value=Dodaj></form>";
      print "<br><br>... lub <a href=mecze.php?akcja=dopisz&ktore=tak><b>dodaj ze wszystkimi klubami</b></a>";
   } else {
      if($czy!="tak") {
         ...
         if($ktore!="tak") {
            $pobierz_klub = mysql_query("select*from kluby where rozgrywki='$ktore' OR rozgrywki2='$ktore' order by nazwa");
            $pobierz_klub2 = mysql_query("select*from kluby where rozgrywki='$ktore' OR rozgrywki2='$ktore' order by nazwa");
            $pobierz_kol = mysql_query("select*from kolejki where rozgrywki='$ktore'");
         } else {
            $pobierz_klub = mysql_query("select*from kluby order by nazwa");
            $pobierz_klub2 = mysql_query("select*from kluby order by nazwa");
            $pobierz_kol = mysql_query("select*from kolejki");
         ...
         echo "<select name=para>";
         echo "<option name=para value=\"\">Nie dotyczy</option>";
         while ($rekord = mysql_fetch_array($pobierz_pare)) {
            ...
            echo "<option name=para value=\"$rekord[id]\">$klub1[nazwa] - $klub2[nazwa]</option>";
         }
         echo "</select>";
         echo "</td></tr>";
         echo "<tr><td class=tekst>Kolejka</td><td>";
