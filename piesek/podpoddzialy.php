...
function edytuj($id, $tabela = "podpoddzialy", $nazwa, $adres, $poddzial, $miejsce) {
   if(!$id) {
      ...
      print "<tr><td><b>ID</td><td width=100><b>Nazwa</td><td width=200><b>Adres</td><td><b>Poddział</td></tr>";
      while ($rekord = mysql_fetch_array($pobierz)) {
         echo "<tr><td>$rekord[id]</td><td>$rekord[nazwa]</td><td>$rekord[adres]</td><td>$rekord[poddzial]</td><td><a href=\"podpoddzialy.php?id=$rekord[id]&akcja=edit\">Edycja</a> / <a href=\"podpoddzialy.php?id=$rekord[id]&akcja=delete\">Usuń</a></td></tr>";
      }
      print "</table>";
   } else {
   ...
   echo "<tr><td class=tekst>Poddział</td><td>";
...
function dopisz($czy, $akcja, $nazwa, $adres, $poddzial, $miejsce) {
   if($czy!="tak") {
      ...
      $ktory2 = $ktory1[id]+1;
      print "ID tego podpoddziału będzie oznaczone numerem: $ktory2";
      ...
      if($nowy_wpis) {
         ...
         echo "Dodałem do poddziału <b>$dzial</b> podpoddział <b>$nazwa</b>.<br>ID podpoddziału to <b>$id[id]</b>.<br><a href=podpoddzialy.php>Powrót</a>";
      } else {
         echo "Nie udało się dodać podpoddziału <b>$nazwa</b>.<br>".mysql_error();
      }
   }
}
