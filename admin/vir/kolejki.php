...
function edytuj($id, $tabela = "kolejki", $numer, $dzien1, $dzien2, $miesiac, $rok, $sezon, $rozgrywki, $s) {
   if(!$id) {
      ...
      $pobierz = mysql_query("select*from kolejki where sezon='$sez' AND rozgrywki='48' order by sezon,numer,rok,miesiac");
   ...
   print "<tr><td class=tekst>Data</td><td class=tekst><input type=text name=dzien1 value=\"$rekord[dzien1]\" size=1> / <input type=text name=miesiac value=\"$rekord[miesiac]\" size=1> / <input type=text name=rok value=\"$rekord[rok]\" size=1> r.</td></tr>\n";
...
function wyswietl() {
   $pobierz = mysql_query("select*from kolejki where rozgrywki='48' order by sezon desc, numer");
   print "<table border=0>";
...
function dopisz($czy, $akcja, $numer, $dzien1, $dzien2, $miesiac, $rok, $sezon, $rozgrywki) {
   if($czy!="tak") {
      ...
      print "<tr><td class=tekst>Data</td><td class=tekst><input type=text name=dzien1 size=1> / <input type=text name=miesiac size=1> / <input type=text name=rok size=1> r.</td></tr>\n";
